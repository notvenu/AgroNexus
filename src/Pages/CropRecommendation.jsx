...
import { getCropRecommendation } from '../api/ml_models';
...
// --- Component definitions moved outside the main function to prevent re-rendering ---
        ...
        <div className="bg-gray-50 dark:bg-dark-background min-h-screen py-12 px-4">
            <div className="container mx-auto max-w-7xl">
                <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
                    <h1 className="text-4xl font-extrabold text-gray-900 dark:text-dark-text text-center mb-4">{t('nav.crop_recommendation')}</h1>
                    <p className="text-lg text-gray-600 dark:text-dark-text-muted text-center mb-8">{t('prediction_pages.crop_rec_subtitle')}</p>
                </motion.div>
                        ...
                        >
                            ...
                            {isLoading ? (
                                <div className="text-center py-8">
                                    <p className="dark:text-dark-text-muted">{t('prediction_pages.analyzing_data')}</p>
                                </div>
                            ) : result ? (
                                <div className="text-center">
                                    <Leaf size={48} className="mx-auto text-green-500 mb-4" />
                                    <p className="text-lg text-gray-600 dark:text-dark-text-muted">{t('prediction_pages.recommended_crop_is')}</p>
                                    ...
                                    <div className="mt-4 bg-gray-100 dark:bg-gray-700 p-3 rounded-lg">
                                        <p className="font-semibold">{t('prediction_pages.confidence')} <span className="font-bold text-green-600">{result.confidence}%</span></p>
                                    </div>
                                </div>
                            ) : (
                                <div className="text-center py-8">
                                    <p className="text-gray-500 dark:text-dark-text-muted">{t('prediction_pages.results_appear_here')}</p>
                                </div>
                            )}
                        </motion.div>
                    )}
